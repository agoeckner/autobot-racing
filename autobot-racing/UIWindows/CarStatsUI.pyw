...
class CarStatsUI(): #{
	...
	def createWindow(self): #{
		...
		# self.addNewCarCallback("RED", "192.168.2.2", "4000", 1, 1)
		#----------------------------------------------------------------------------------------------------------------------------

		self.window.bind("<Configure>", self.updateWindow)
		self.window.after(5000, self.doTelem)
	...
	def doTelem(self):
		self.parent.parent.runTelem()
		self.window.after(1000, self.doTelem)  # reschedule event in 2 seconds
